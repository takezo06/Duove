...
export function CycleLog() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 3. Tell tracker to refresh
      window.dispatchEvent(new Event('cycle-logged'));

      // 4. Back to tracker
      localStorage.setItem('cycle-refresh', Date.now().toString());
      navigate('/cycle');
    } catch (err: any) {
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="space-y-4">
        {Object.entries(categoryConfig).map(([key, config]) => {
          ...
          return (
            <div key={key} className="bg-neutral-800/30 rounded-xl border border-neutral-700/50 overflow-hidden">
              <button
                        ...
                        <div className="flex gap-2 mt-1">
                          {['low', 'medium', 'high'].map(level => (
                            <button
                              ...
                              onClick={() =>
                                setStress(
                                  stress === level ? null : (level as 'low' | 'medium' | 'high')
                                )
                              }
                              className={`px-3 py-1.5 rounded-full text-xs border transition ${
                                stress === level
